...
The wordmark sizes itself fluidly: `clamp(60px, 20.5vw, 80px)`. Monoton renders roughly 3.9× the font size wide, so any fixed size that fits a 390px phone would overflow a 320px one. The `vw` term only does work below roughly 390px — from there up the wordmark holds at the 80px cap rather than growing with the viewport. `line-height: 1` trims Monoton's generous line box so the 24px gap to the tagline is the real gap rather than 24px plus leading. Only the tagline steps at the 600px/400px breakpoints.
